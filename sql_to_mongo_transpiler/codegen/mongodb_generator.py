    ...
    def _has_aggregate(self, node):
        ...
        if hasattr(node, "order_by") and node.order_by:
            for item in node.order_by:
                if isinstance(item.column, Aggregate):
                    return True
        return False
    ...
    def _generate_aggregate(self, node):
        ...
        if node.order_by:
            ...
            for item in node.order_by:
                ...
                if isinstance(item.column, Aggregate):
                    func = item.column.func
                    col_arg = item.column.column
                    if func == "COUNT":
                        field = "count" if col_arg == "*" else f"count_{col_arg}"
                    else:
                        field = f"{func.lower()}_{col_arg}"
                    sort_doc[field] = direction
                # If sorting by grouped column → map to _id
                elif node.group_by and item.column in node.group_by:
                    if len(node.group_by) == 1:
