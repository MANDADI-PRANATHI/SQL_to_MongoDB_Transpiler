...
def transpile(schema: dict, query: str) -> dict:
    """
    Core transpiler wrapper.
    Converts SQL to MongoDB Query JSON.
    """
    try:
...
def interactive_shell(args):
    ...
    while True:
        try:
            ...
            result = transpile(schema, sql)
            
            if not isinstance(result, dict) or "string" not in result:
                print("Error: Transpiler did not return a valid MongoDB query string.")
                continue
            
            output_str = result["string"]
            print(output_str)
...
def main():
    ...
    if args.command == "transpile":
        ...
        if not isinstance(result, dict) or "string" not in result:
            print("Error: Transpiler did not return a valid MongoDB query string.", file=sys.stderr)
            sys.exit(1)
